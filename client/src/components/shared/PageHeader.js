...
function Header() {
  ...
  return (
    <>
      {/* Header */}
      <header  className={`w-full left-0 top-0 z-50 py-4 transition-all duration-200 ease-in-out fixed ${showHeader ? 'bg-white text-black shadow-md' : 'text-white'}`}>
  <nav className="container h-18 mx-auto flex items-center justify-between px-2 md:px-14 gap-4">
    ...
    <div className="flex md:hidden items-center transform translate-x-4 translate-y-1">
      ...
      >
        <div className={`h-1 w-8 rounded-full transition-all duration-700 ${mobileMenuOpen ? 'bg-black animate-translate-rotate' : 'bg-white animate-reverse-translate-rotate'}`}></div>
        <div className={`h-1 w-8 rounded-full transition-opacity delay-300 ease-in ${mobileMenuOpen ? 'opacity-0' : 'opacity-100 bg-white'}`}></div>
        <div className={`h-1 w-8 rounded-full transition-all duration-700 ${mobileMenuOpen ? 'bg-black animate-translate-rotate2' : 'bg-white animate-reverse-translate-rotate2'}`}></div>
      </div>
    </div>
